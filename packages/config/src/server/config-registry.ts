 ...
 * ```ts
 * const shopCfg = configOf(core).of<ShopConfigDef>('vendor_shop', { actorId: player.id });
...
import { Announcement, authorize, isRecord, isUsable } from '@bedrock-core/server-runtime';
...
/**
 * Typed view of another addon's config. Obtain via `configOf(core).of<I>()` or
 * `configOf(core).subscribe<I>()`. Reads and writes go over RPC; `patch` merges a
